...
public:
    ...
    inline bool contains(const point2d& pts) {
        return in_range(pts.x, left(), right()) && in_range(pts.y, top(), bottom());
    }
};
...
public:
    ...
    inline std::vector<vector2d> points() const {
        return _pts;
    }
    inline std::vector<vector2d> normals() const {
        return _nms;
    }
...
public:
    ...
    static void compute_nms(const std::vector<vector2d>& pts,
                            std::vector<vector2d>&       nms);
};
