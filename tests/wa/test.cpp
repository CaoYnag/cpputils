...
struct marked_point {
    ...
    marked_point(const point2d& p, u32 s) :
        pt(p), stat(s), t(-1), w(-1) {
    }
    marked_point(const point2d& p, u32 s, f32 tt, f32 ww) :
        pt(p), stat(s), t(tt), w(ww) {
    }
};
enum WA_MARKS {
    WA_EMPTY = 0,
...
int weiler_atherton(const polygon2d& target, const polygon2d& window, vector<polygon2d>& rslt) {
    ...
    for (int we = 0; we < window.num(); ++we) {
        ...
        for (int te = 0; te < target.num(); ++te) {
            ...
            if (v > 0)
                stat |= WA_OUT; // all normals goes out, so if the dot result is bigger than 0, so this line goes out.
            else
                stat |= WA_IN;
